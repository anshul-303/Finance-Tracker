...
import { loadExpenseByCategory } from "../../fetchApi/DashboardApi/dashboardApi";
...
export default function ExpensePieChart() {
  ...
  useEffect(() => {
    const fetchExpenseData = async () => {
      const resData = await loadExpenseByCategory();
...
const CustomTooltip = ({ active, payload }) => {
  ...
  if (notEmptyData)
    return (
      <div className="md:w-[49%] w-[99%] md:min-h-[40vh] lg:min-h-[60vh] h-[45vh] border rounded-sm flex flex-col justify-center items-center md:mb-4 mb-5  md:gap-7 lg:gap-2 gap-5 ">
        <h3 className="text-sm font-semibold text-zinc-300 px-4">
          Lifetime Expense Distribution
        </h3>
        <div className="mb-2">
          <PieChart width={300} height={280}>
            <Pie data={expenseData} nameKey="category"  dataKey="total" label>
